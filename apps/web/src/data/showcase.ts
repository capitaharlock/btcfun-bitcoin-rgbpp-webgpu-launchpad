 ...
 * earned (`reward` in `lib/standard.ts`), and no tally has a mint in a
 * halving the launch has not reached. `showcase.test.ts` holds them to that.
 */

import type { TokenArt } from "../lib/launches/image";
import { HALVING_BLOCKS, reward } from "../lib/standard";
import { place, type Launch, type Placement } from "./launches";
