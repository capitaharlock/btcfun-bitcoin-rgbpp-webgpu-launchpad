...
import { alice, launches, network, provider, register, vaultOf } from "../../kit.mjs";
...
export async function announce() {
  ...
  for (const [symbol, name, blurb, accent] of OFFICIAL) {
    ...
    const draft = {
      ...
      links: Object.fromEntries(launches.LINK_KINDS.map((k) => [k, ""])),
      story: { why: "", plan: "" },
      image: EXTRAS[symbol]?.[3] ?? "",
    };
    const faults = launches.validate(draft, network.ACTIVE);
    ...
    write();
    const commitment = launches.commitmentFor(draft, identity, admitted.registration, admitted.certificate, network.ACTIVE);
    if (!launches.idMatches(commitment, network.ACTIVE)) throw new Error(`${symbol}: the certificate does not verify`);
    await publish(alice, {
      kind: "launch",
      launch: commitment.id,
      ref: launches.commitmentId(commitment),
