...
function word(layer, x, y, text, c) {
...
const BTC = ["..#.#..", "######.", ".##..##", ".##..##", ".#####.", ".##..##", ".##..##", "######.", "..#.#.."];
const btc = (layer, x, y, c) => layer.stamp(x, y, BTC, { "#": c });

// ── composition ──────────────────────────────────────────────────────────────

function sparkle(layer, x, y, c, big = false) {
  layer.px(x, y, c).px(x - 1, y, c).px(x + 1, y, c).px(x, y - 1, c).px(x, y + 1, c);
  if (big) layer.px(x - 2, y, c).px(x + 2, y, c).px(x, y - 2, c).px(x, y + 2, c);
}

function compose({ accent, draw, fx, sparkles = [] }) {
