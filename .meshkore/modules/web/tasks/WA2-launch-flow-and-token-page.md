...
`apps/web/src/pages/create/` asks only for identity, income address and opening; `apps/web/src/pages/Launch.tsx` shows the standard terms, the opening block, the promoter, the token id, supply read from CKB cells (labelled cells, not people) and the reward by halving. Covered by `e2e/ui/create-wizard.spec.ts` and `e2e/ui/time.spec.ts`. Reserve denomination and expired allowance no longer exist under the standard tokenomics.
