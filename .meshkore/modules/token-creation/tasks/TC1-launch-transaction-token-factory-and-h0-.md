...
A launch is its terms: the token's type hash is derived from the mint script's args, which commit to `h0`, the promoter and the metadata (`apps/web/src/lib/rgbpp/launch.ts`), and anyone can create one without permission (`apps/web/src/views/Create.tsx`, `apps/web/src/lib/launches/create.ts`). Issuance before `h0` is refused on chain (`nothing_is_minted_for_a_ticket_anchored_before_the_launch_opens`, `contracts/tests/src/mint.rs`). No reserve asset is named: the standard has none ([decision](../../../context/decisions/2026-09-24-standard-tokenomics-and-instant-mint.md)).
