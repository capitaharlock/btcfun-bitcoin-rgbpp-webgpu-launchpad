...
`apps/web/src/components/diagram/`: `model.ts` (diagrams as data — lanes, terminal/process/decision/note/cell/transaction nodes, flow/yes/no/seal/commit edges), `layout.ts` (pure geometry: row stacking, orthogonal routing with nested gutters, byte strips), `text.ts` (DOM-free wrapping), `Diagram.tsx` (SVG with `role="img"`, title and description, arrowheads, legend for dashed relations, sideways scroll inside the card below 78 % scale) and `diagram.css`. The pastel palette is `--dg-*` tokens in `src/ui/tokens.css`, mixed from the site's accents on a white card in every theme. `layout.test.ts` covers routing, gutters, wrapping and overlap; the browser suite checks every label for WCAG AA contrast against the shape it sits on.
